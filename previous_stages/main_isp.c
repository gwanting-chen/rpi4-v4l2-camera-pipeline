...
int main(){
	...
	struct my_buffer* cam_bufs = request_and_map_buffers(cam_fd, 4, V4L2_BUF_TYPE_VIDEO_CAPTURE);
	struct my_buffer* isp_out_bufs = request_and_map_buffers(isp_out_fd, 4, V4L2_BUF_TYPE_VIDEO_OUTPUT);
...
*/
	...
	for(int i=0;i < last;i++){
		...
		queue_buffers(isp_out_fd, &isp_out_buf, type_isp_out, out_idx,cam_buf.bytesused);
		...
		dequeue_buffers(isp_out_fd, &isp_out_buf, type_isp_out);
