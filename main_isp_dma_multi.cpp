...
void ae_awb_worker(V4L2Node* sensor_node,V4L2Node* isp_out_node){
	...
	const float TARGET_RATIO = 0.18f;//18% middle gray photography standard
	...
	while(g_ctx.is_running){
		...
		//Tuned P-controller gain for smooth convergence
		float Kp_exp = 0.5f;
		float Kp_gain = 0.05f;
...
int main() {
	...
	if(cam_node.setFormat(V4L2_PIX_FMT_SRGGB10,width,height) < 0){

		std::cerr << "[Fatal Error] Failed to set format for Camera Node. Exiting..." << std::endl;
	        return -1;
	}

	if(isp_out_node.setFormat(V4L2_PIX_FMT_SRGGB10,width,height) < 0){

		std::cerr << "[Fatal Error] Failed to set format for ISP Output Node. Exiting..." << std::endl;
        	return -1;
	}

	if(isp_cap_node.setFormat(V4L2_PIX_FMT_RGB24,width,height) < 0){

		std::cerr << "[Fatal Error] Failed to set format for ISP Capture Node. Exiting..." << std::endl;
		return -1;

	}

	if(isp_meta_node.setMetaFormat(V4L2_META_FMT_BCM2835_ISP_STATS) < 0){

		std::cerr << "[Fatal Error] Failed to set meta format for ISP Meta Node. Exiting..." << std::endl;
		return -1;
	...
	if(cam_node.requestAndExportBuffers(buf_count) < 0){

		std::cerr << "[Fatal Error] Failed to request/export buffers for Camera Node. Exiting..." << std::endl;
		return -1;
	}

	if(isp_out_node.requestDmaBuffers(buf_count) < 0){

		std::cerr << "[Fatal Error] Failed to request DMA buffers for ISP Output Node. Exiting..." << std::endl;
		return -1;
	}

	if(isp_cap_node.requestAndMapBuffers(buf_count) < 0){

		std::cerr << "[Fatal Error] Failed to request/map buffers for ISP Capture Node. Exiting..." << std::endl;
		return -1;
	}

	if(isp_meta_node.requestAndMapBuffers(buf_count) < 0){

		std::cerr << "[Fatal Error] Failed to request/map buffers for ISP Meta Node. Exiting..." << std::endl;
		return -1;
	}
